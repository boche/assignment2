...
__global__ void set0_scan(int N, int* output) {
	output[N - 1] = 0;
}

__global__ void downsweep_scan(int twod, int N, int* output) {
	int twod1 = twod * 2;
	int idx = (blockIdx.x * blockDim.x + threadIdx.x) * twod1;
	if (idx + twod1 - 1 < N) {
		int t = output[idx + twod - 1];
		output[idx + twod - 1] = output[idx + twod1 - 1];
		output[idx + twod1 - 1] += t;
	}
}
...
void exclusive_scan(int* device_start, int length, int* device_result)
{
	...
	// no need to copy to device_result, since it's already set in cudaScan
	/*cudaMemcpy(device_result, device_start, N * sizeof(int), cudaMemcpyDeviceToDevice);*/
	for (int twod = 1; twod < N; twod *= 2) {
		int nop = N / (twod * 2);
		upsweep_scan<<< (nop  + threadsPerBlock - 1) / threadsPerBlock, 
			threadsPerBlock>>>(twod, N, device_result);
		//need sync??
	}
	set0_scan<<<1, 1>>>(N, device_result);
	for (int twod = N / 2; twod >= 1; twod /= 2) {
		int nop = N / (twod * 2);
		// this part can be further sped up because in the end, we are using 
		// only one thread block
		downsweep_scan<<< (nop  + threadsPerBlock - 1) / threadsPerBlock, 
			threadsPerBlock>>>(twod, N, device_result);
	}
}
...
void debug(int* a, int N) {
	int *tmp = new int[N];
	cudaMemcpy(tmp, a, N * sizeof(int), cudaMemcpyDeviceToHost);
	for (int i = 0; i < N; i++) printf("%02d ", i);
	printf("\n");
	for (int i = 0; i < N; i++) printf("%02d ", tmp[i]);
	printf("\n");
}
...
int find_repeats(int *device_input, int length, int *device_output) {
	...
	cudaMemcpy(device_output, device_input, N * sizeof(int), cudaMemcpyDeviceToDevice);
	// set input to 1 if if repeat
	is_repeat<<<(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock>>>(N, device_output, device_input);
	// don't use device_start anyway, accumulate 1 in device input
	exclusive_scan(NULL, length, device_input);
	fill_idx<<<(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock>>>(N, device_input, device_output);
